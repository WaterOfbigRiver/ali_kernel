			      ...
			      unsigned long *bytes, unsigned long *usec)
{
 ...
 fail:
	pr_warning("Failed to parse network rate limit. Traffic unlimited.\n");
	kfree(ratestr);
}
		...
		const char **vec, unsigned int len)
{
	...
	pr_debug("rate changed\n");
	if (be && be->dev && be->vif && be->dev->nodename) {
		xen_net_read_rate(be->dev, &be->vif->credit_bytes,
			&be->vif->credit_usec);
		if (atomic64_read(&be->vif->remaining_credit) > NEED_CHANGED_BYTES) {
			atomic64_set(&be->vif->remaining_credit, be->vif->credit_bytes);
		}
		xenbus_write(XBT_NIL, be->dev->nodename, "rate_status", "changed");
	} else if (be && be->dev && be->dev->nodename) {
		xenbus_write(XBT_NIL, be->dev->nodename, "rate_status", "unknown");	
	}
}
