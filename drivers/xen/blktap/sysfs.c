...
#define blktap_sysfs_show_devices(start)                           		\
static ssize_t                                                     		\
blktap_sysfs_show_devices##start(struct class *class, char *buf)   		\
{                                                                              \
	int i, ret, end;                                                        \
	struct blktap *tap;                                                     \
	                                                                        \
	ret = 0;                                                    		\
	i = start * DEVICE_PER_FILE;                                            \
	end = i + DEVICE_PER_FILE;                                              \
	for (; i < MAX_BLKTAP_DEVICE && i < end; i++){                          \
		tap = blktaps[i];                                               \
		if (!tap)                                                       \
		continue;                                                       \
                                                                               \
...
#define class_attr(start) \
    CLASS_ATTR(devices##start, S_IRUSR, blktap_sysfs_show_devices##start, NULL);

#define class_remove_sysfile(start)                             \
    class_remove_file(class, &class_attr_devices##start);

#define class_create_sysfile(start)                             \
    err = class_create_file(cls, &class_attr_devices##start);   \
    if (err)                                                    \
        goto out_unregister;                                    \

#define DEVICE_PER_FILE 20 //each device show 20 devices, totally we need 13 devices
blktap_sysfs_show_devices(0); 
blktap_sysfs_show_devices(1); 
blktap_sysfs_show_devices(2); 
blktap_sysfs_show_devices(3); 
blktap_sysfs_show_devices(4); 
blktap_sysfs_show_devices(5); 
blktap_sysfs_show_devices(6); 
blktap_sysfs_show_devices(7); 
blktap_sysfs_show_devices(8); 
blktap_sysfs_show_devices(9); 
blktap_sysfs_show_devices(10); 
blktap_sysfs_show_devices(11); 
blktap_sysfs_show_devices(12); 
...
blktap_sysfs_free(void)
{
	...
	class_remove_sysfile(0)
	class_remove_sysfile(1)
	class_remove_sysfile(2)
	class_remove_sysfile(3)
	class_remove_sysfile(4)
	class_remove_sysfile(5)
	class_remove_sysfile(6)
	class_remove_sysfile(7)
	class_remove_sysfile(8)
	class_remove_sysfile(9)
	class_remove_sysfile(10)
	class_remove_sysfile(11)
	class_remove_sysfile(12)
...
blktap_sysfs_init(void)
{
	...
	class_create_sysfile(0)
	class_create_sysfile(1)
	class_create_sysfile(2)
	class_create_sysfile(3)
	class_create_sysfile(4)
	class_create_sysfile(5)
	class_create_sysfile(6)
	class_create_sysfile(7)
	class_create_sysfile(8)
	class_create_sysfile(9)
	class_create_sysfile(10)
	class_create_sysfile(11)
	class_create_sysfile(12)
